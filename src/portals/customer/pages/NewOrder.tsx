    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 p-0 sm:items-center sm:p-4" role="presentation" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-labelledby="new-order-title" onMouseDown={(event) => event.stopPropagation()} className="relative max-h-[94vh] w-full overflow-y-auto rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl sm:max-w-3xl sm:p-8">
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm">
          <div className="flex items-start justify-between gap-2"><div className="min-w-0"><h3 className="text-lg font-bold text-slate-900">{isReadOnly ? 'Laundry items' : 'Add laundry items'}</h3><p className="mt-1 text-sm text-slate-500">{isReadOnly ? 'The items and service details for this order.' : 'Choose a category, service and quantity. Add another row for more items.'}</p></div><span className="shrink-0 whitespace-nowrap rounded-full bg-brand-soft px-2.5 py-1 text-xs font-medium text-brand-primary">{displayItems.length} item type{displayItems.length === 1 ? '' : 's'}</span></div>
          ...
          {displayItems.length > 0 && <div className="mt-5 divide-y divide-slate-100 rounded-2xl border border-slate-200">{displayItems.map((item, index) => <div key={`${item.category}-${item.service}-${index}`} className="flex flex-col gap-3 p-3 text-sm sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="font-semibold text-slate-900">{item.category}</p><p className="text-xs text-slate-500">{item.service} · {item.quantity} item{item.quantity === 1 ? '' : 's'}</p></div><div className="flex items-center gap-2 self-stretch sm:self-auto">{!isReadOnly && <input type="number" min={1} inputMode="numeric" aria-label={`Quantity for ${item.category}`} value={item.quantity} onChange={(event) => setItems((currentItems) => currentItems.map((currentItem, itemIndex) => itemIndex === index ? { ...currentItem, quantity: Math.max(1, Number(event.target.value) || 1) } : currentItem))} className="w-16 rounded-xl border border-slate-200 px-2 py-1 text-center font-semibold text-slate-900 focus:border-brand-primary focus:ring-2 focus:ring-brand-focus" />}<span className="min-w-0 flex-1 text-right font-semibold text-slate-900">{subscription ? `${item.quantity * (item.subscriptionUnits ?? 1)} unit${item.quantity * (item.subscriptionUnits ?? 1) === 1 ? '' : 's'}` : `₦${(item.quantity * item.unitPrice).toLocaleString()}`}</span>{!isReadOnly && <button type="button" aria-label={`Remove ${item.category}`} onClick={() => setItems((currentItems) => currentItems.filter((_, itemIndex) => itemIndex !== index))} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl text-slate-400 hover:bg-brand-soft hover:text-brand-primary"><Trash2 className="h-4 w-4" /></button>}</div></div>)}</div>}
        </section>
