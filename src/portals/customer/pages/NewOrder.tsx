    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 sm:items-center sm:p-4" role="presentation" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-labelledby="new-order-title" onMouseDown={(event) => event.stopPropagation()} className="relative max-h-[94vh] w-full overflow-y-auto rounded-2xl border border-[#e7e7e7] bg-white p-4 shadow-xl sm:max-w-3xl sm:p-6">
        ...
        <header className="flex items-start justify-between gap-3">
          <div><h2 id="new-order-title" className="text-2xl font-bold tracking-tight text-[#121212]">{isReadOnly ? 'Order details' : 'Create an order'}</h2><p className="mt-1 text-sm text-slate-500">{isReadOnly ? <><CopyableOrderId id={order?.id ?? ''} /> · {order?.status}</> : 'Add each type of item and choose how you want it cared for.'}</p></div>
          <button type="button" onClick={onClose} aria-label={isReadOnly ? 'Close order details' : 'Close new order'} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-white text-xl text-slate-500 transition hover:border-brand-border hover:text-brand-primary">×</button>
        </header>

        {isReadOnly && <section className="mt-5 grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-[#e7e7e7] bg-slate-50 p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400">Payment status</p><p className="mt-2 font-semibold text-slate-900">{order?.paymentStatus}</p></div>
          <div className="rounded-2xl border border-[#e7e7e7] bg-slate-50 p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400">Customer ID</p><p className="mt-2 font-semibold text-slate-900">{order?.customerId}</p></div>
          <div className="rounded-2xl border border-[#e7e7e7] bg-slate-50 p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400">Pickup OTP</p><div className="mt-2 flex gap-1.5">{Array.from({ length: 5 }).map((_, index) => <span key={`pickup-${index}`} className="flex h-9 w-9 items-center justify-center rounded-md border border-[#ff4a4a] bg-white text-sm font-bold text-[#ff4a4a]">{order?.pickupOtp[index] ?? ''}</span>)}</div></div>
          <div className="rounded-2xl border border-[#e7e7e7] bg-slate-50 p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400">Delivery OTP</p><div className="mt-2 flex gap-1.5">{Array.from({ length: 5 }).map((_, index) => <span key={`delivery-${index}`} className="flex h-9 w-9 items-center justify-center rounded-md border border-[#ff4a4a] bg-white text-sm font-bold text-[#ff4a4a]">{(order?.deliveryOtp ?? '').replace(/\D/g, '')[index] ?? ''}</span>)}</div></div>
          <div className="rounded-2xl border border-[#e7e7e7] bg-slate-50 p-4 sm:col-span-2"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400">Pickup date</p><p className="mt-2 font-semibold text-slate-900">{order?.pickupDate ?? 'Not confirmed yet'}</p></div>
        </section>}
        ...
        <section className="mt-4 rounded-2xl border border-[#e7e7e7] p-4 sm:p-5"><label><span className="mb-1.5 block text-sm font-medium text-slate-600">Pickup instructions <span className="font-normal text-slate-400">(optional)</span></span><textarea rows={3} placeholder="Separate whites, handle silk carefully..." value={order?.notes ?? notes} onChange={(event) => setNotes(event.target.value)} readOnly={isReadOnly} className="w-full resize-none rounded-2xl border border-slate-200 bg-white px-3 py-3 text-base text-slate-900 placeholder:text-slate-400 focus:border-brand-primary focus:ring-2 focus:ring-brand-focus read-only:cursor-default read-only:bg-slate-50" /></label><label className="mt-4 block"><span className="mb-1.5 block text-sm font-medium text-slate-600">Pickup location</span><select value={order?.pickupLocation ?? pickupLocation} onChange={(event) => setPickupLocation(event.target.value)} disabled={isReadOnly} className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-3 text-base text-slate-900 focus:border-brand-primary focus:ring-2 focus:ring-brand-focus disabled:cursor-default disabled:bg-slate-50 disabled:opacity-100">{pickupLocations.map((location) => <option key={location.id} value={location.name}>{location.name}</option>)}{order && !pickupLocations.some((location) => location.name === order.pickupLocation) && <option>{order.pickupLocation}</option>}</select></label></section>

        {!isReadOnly && subscription && <p className="mt-4 rounded-2xl bg-brand-soft p-3 text-sm text-brand-strong">{subscriptionRemainingUnits} weighted units remain on your {subscription.name} plan this week.</p>}

        <section className="mt-4 rounded-2xl border border-[#a7d7d2] bg-[#eef9f7] p-5 text-slate-900 sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#418d87]">{isReadOnly ? 'Order total' : 'Order estimate'}</p><p className="mt-2 text-sm text-slate-600">{subscription ? `${order ? itemCount : weightedItemCount} weighted unit${(order ? itemCount : weightedItemCount) === 1 ? '' : 's'}` : `${itemCount} item${itemCount === 1 ? '' : 's'}`} across {displayItems.length} item type{displayItems.length === 1 ? '' : 's'}</p></div>{!subscription && <p className="text-2xl font-bold">₦{total.toLocaleString()}</p>}</div>{isReadOnly ? <button type="button" onClick={onClose} className="mt-5 w-full rounded-2xl bg-brand-primary px-4 py-3 text-sm font-semibold text-white transition hover:bg-brand-primary-hover">Close details</button> : <button type="button" disabled={items.length === 0 || isSaving} onClick={handleCreateOrder} className="mt-5 w-full rounded-2xl bg-brand-primary px-4 py-3 text-sm font-semibold text-white transition hover:bg-brand-primary-hover disabled:cursor-not-allowed disabled:opacity-50">{isSaving ? 'Saving order...' : 'Continue to pickup details'}</button>}</section>
      </section>
    </div>
  )
}