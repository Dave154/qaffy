...
export async function action({ request }: Route.ActionArgs) {
  ...
  const [order] = await sql`
    ...
    where id = ${orderId}
      and status in ('picked_up', 'at_vendor', 'invoiced', 'paid')
      and right(regexp_replace(coalesce(delivery_otp, ''), '[^0-9]', '', 'g'), 4) = ${otp}
