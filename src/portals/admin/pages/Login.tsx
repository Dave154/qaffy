...
export default function Login() {
  ...
  useEffect(() => {
    const redirectExistingAdmin = async () => {
      ...
      const { data: role } = await supabase.from('profile_roles').select('role').eq('profile_id', userData.user.id).eq('role', 'admin').eq('status', 'approved').maybeSingle()
  ...
  const signInWithGoogle = async () => {
    setError('')
    if (!isSupabaseConfigured || !supabase) return setError('Supabase is not configured. Add the required environment variables to continue.')
    setIsSubmitting(true)
