...
export default function Home() {
  ...
  const cards = [{ label: 'Gross revenue', value: money(metrics.revenue), helper: 'Paid invoices', icon: CircleDollarSign, href: '/admin/orders', tone: 'green' }, { label: 'Vendor payouts', value: money(metrics.vendorPayouts), helper: 'Recorded settlements', icon: Store, href: '/admin/partners/vendors', tone: 'pink' }, { label: 'Platform profit', value: money(metrics.platformProfit), helper: 'Revenue less settlements', icon: CircleDollarSign, href: '/admin/orders', tone: 'green' }, { label: 'Subscription revenue', value: money(metrics.subscriptionRevenue), helper: 'Paid subscription orders', icon: CircleDollarSign, href: '/admin/orders', tone: 'green' }, { label: 'One-time revenue', value: money(metrics.oneTimeRevenue), helper: 'Paid one-time orders', icon: CircleDollarSign, href: '/admin/orders', tone: 'green' }, { label: 'One-time orders', value: metrics.oneTimeOrders, helper: `${metrics.totalOrders} total orders`, icon: Package, href: '/admin/orders', tone: 'pink' }, { label: 'Total vendors', value: metrics.vendors, helper: 'Approved vendors', icon: UsersRound, href: '/admin/partners/vendors', tone: 'pink' }]
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.6fr_1fr]"><section className="rounded-xl border border-[#e8eeee] bg-white p-5 shadow-[0_5px_18px_rgba(21,61,73,0.04)]"><div className="flex items-start justify-between"><h3 className="text-lg font-bold text-slate-900">Orders over the last 7 days</h3><div className="flex items-center gap-3 text-[11px] text-slate-500"><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#00b7d4]" />Orders</span><span className="rounded-md bg-slate-50 px-2 py-1">This week</span></div></div><div className="mt-5"><TrendChart points={trend} /></div><div className="mt-4 flex items-center justify-between rounded-lg bg-[#f8fcfc] px-3 py-2 text-xs"><span className="text-slate-500">Total in period</span><strong className="text-slate-900">{trend.reduce((sum, point) => sum + point.orders, 0)} orders</strong></div></section><section className="flex flex-col rounded-xl border border-[#e8eeee] bg-white p-5 shadow-[0_5px_18px_rgba(21,61,73,0.04)]"><div className="flex items-start justify-between"><h3 className="text-lg font-bold text-slate-900">Revenue</h3><CircleDollarSign size={19} className="text-[#64c4ae]" /></div><div className="mt-6 flex items-end gap-2"><p className="text-3xl font-bold text-slate-900">{money(metrics.revenue)}</p><span className="mb-1 rounded-full bg-[#eafaf4] px-2 py-1 text-[10px] font-semibold text-[#3a9a82]">Collected</span></div><div className="mt-auto flex h-24 items-end gap-2">{trend.map((point) => <div key={point.label} className="flex flex-1 flex-col items-center gap-2"><div className="flex h-16 w-full items-end rounded-md bg-[#f1faf8]"><div className="w-full rounded-md bg-[#64c4ae]" style={{ height: `${Math.max((point.revenue / Math.max(...trend.map((item) => item.revenue), 1)) * 100, point.revenue ? 8 : 2)}%` }} /></div><span className="text-[10px] text-slate-400">{point.label}</span></div>)}</div></section></div>
    <div className="grid gap-5 xl:grid-cols-[1fr_1fr_1fr]"><section className="rounded-xl border border-[#e8eeee] bg-white p-5 shadow-[0_5px_18px_rgba(21,61,73,0.04)]"><div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-bold text-slate-900">Workload by status</h3><BarChart3 size={18} className="text-[#00a7bd]" /></div><PipelineChart pipeline={pipeline} /></section><section className="rounded-xl border border-[#e8eeee] bg-white p-5 shadow-[0_5px_18px_rgba(21,61,73,0.04)]"><div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-bold text-slate-900">Plans</h3><Link to="/admin/plans" className="text-xs font-semibold text-[#008fa6]">Manage</Link></div><div className="space-y-3">{plans.length === 0 ? <p className="text-sm text-slate-500">No plans configured.</p> : plans.slice(0, 4).map((plan) => <div key={plan.id} className="flex items-center justify-between"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800">{plan.name} <span className="font-normal text-slate-400">{plan.type}</span></p><div className="mt-1 h-1.5 w-32 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#00b7d4]" style={{ width: `${Math.min(plan.subscribers * 18 + 8, 100)}%` }} /></div></div><strong className="text-sm text-slate-900">{plan.subscribers}</strong></div>)}</div></section><section className="rounded-xl border border-[#e8eeee] bg-white p-5 shadow-[0_5px_18px_rgba(21,61,73,0.04)]"><div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-bold text-slate-900">Recent updates</h3><Clock3 size={18} className="text-slate-400" /></div><div className="space-y-3">{activity.length === 0 ? <p className="text-sm text-slate-500">No recent activity.</p> : activity.map((item) => <div key={item.id} className="flex gap-3"><span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${item.tone === 'cyan' ? 'bg-[#00b7d4]' : item.tone === 'pink' ? 'bg-[#ff6077]' : 'bg-[#64c4ae]'}`} /><div className="min-w-0"><p className="truncate text-xs font-semibold text-slate-800">{item.title}</p><p className="truncate text-[11px] text-slate-500">{item.detail}</p><p className="mt-0.5 text-[10px] text-slate-400">{item.time}</p></div></div>)}</div></section></div>
    <div className="grid gap-5 xl:grid-cols-2"><section className="rounded-xl border border-[#e8eeee] bg-white p-5 shadow-[0_5px_18px_rgba(21,61,73,0.04)]"><div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-bold text-slate-900">Recent vendors</h3><Store size={18} className="text-brand-primary" /></div><div className="space-y-3">{recentVendors.length === 0 ? <p className="text-sm text-slate-500">No vendors registered.</p> : recentVendors.map((vendor) => <div key={vendor.id} className="flex items-center justify-between gap-3 border-b border-slate-100 pb-3 last:border-0 last:pb-0"><p className="truncate text-sm font-semibold text-slate-800">{vendor.name}</p><span className="shrink-0 rounded-full bg-brand-soft px-2 py-1 text-[10px] font-semibold capitalize text-brand-primary">{vendor.status}</span></div>)}</div></section><section className="rounded-xl border border-[#e8eeee] bg-white p-5 shadow-[0_5px_18px_rgba(21,61,73,0.04)]"><div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-bold text-slate-900">Recent customers</h3><UserRound size={18} className="text-brand-primary" /></div><div className="space-y-3">{recentCustomers.length === 0 ? <p className="text-sm text-slate-500">No customers registered.</p> : recentCustomers.map((customer) => <div key={customer.id} className="flex items-center justify-between gap-3 border-b border-slate-100 pb-3 last:border-0 last:pb-0"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800">{customer.name}</p><p className="truncate text-xs text-slate-500">{customer.email ?? 'Email unavailable'}</p></div><span className="shrink-0 text-xs font-semibold text-slate-500">Customer</span></div>)}</div></section></div>
  </div>
}
