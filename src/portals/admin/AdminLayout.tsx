...
export default function AdminLayout() {
  ...
  const [isPartnersOpen, setIsPartnersOpen] = useState(location.pathname.startsWith('/admin/partners'))
      ...
      <aside className={`fixed left-0 top-0 z-30 hidden h-screen max-h-screen shrink-0 self-start flex-col overflow-visible border-r border-[#f2f3f3] bg-white py-7 transition-[width] duration-300 md:flex ${isCollapsed ? 'w-[76px] px-3' : 'w-[221px] px-[13px]'}`}>
        ...
        <nav className={`mx-auto space-y-[13px] ${isCollapsed ? 'w-full' : 'w-[194px]'}`}>
          {navItems.map((item) => <div key={item.to} className="space-y-1">
            {item.children ? <button type="button" onClick={() => setIsPartnersOpen((open) => !open)} aria-expanded={isPartnersOpen} className={`group relative flex h-10 w-full items-center rounded-[10px] text-sm font-medium transition ${isCollapsed ? 'justify-center px-0' : 'gap-3 px-4'} ${location.pathname.startsWith('/admin/partners') ? 'bg-brand-soft text-brand-primary' : 'text-[#121212] hover:bg-brand-soft hover:text-brand-primary'}`}>
              <item.icon size={17} strokeWidth={1.8} aria-hidden="true" />
              <span className={isCollapsed ? 'sr-only' : ''}>{item.label}</span>
              {!isCollapsed && <ChevronDown size={15} aria-hidden="true" className={`ml-auto transition-transform duration-300 ${isPartnersOpen ? 'rotate-180' : ''}`} />}
